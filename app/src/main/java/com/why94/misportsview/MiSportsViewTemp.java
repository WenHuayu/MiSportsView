 ...
 */
public class MiSportsViewTemp extends View {
    ...
    private Paint mConnectRingStarPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
    private float[] mConnectRingStarPoint;
    public boolean abc;
    ...
    String mContentText = "2274", mInfoText = "1.5公里 | 34千卡";
    int mContentTextSize = DisplayUtils.sp2px(48), mInfoTextSize = DisplayUtils.sp2px(12);
    ...
    public MiSportsViewTemp(Context context) {
        this(context, null);
    }

    public MiSportsViewTemp(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    ...
    private void onDrawWatch(Canvas canvas) {
        Path path = new Path();
    ...
    private void drawConnectedRing(Canvas canvas) {
        ...
        canvas.rotate(angles, mDrawArea.centerX(), mDrawArea.centerY());
    ...
    private void drawConnectRing(Canvas canvas) {
        for (int i = 0, size = mConnectRings.size(); i < size; i++) {
            ...
            drawRotateArc(canvas, mConnectRingPaint, angles + i * CONNECT_RING_DEGREES_OFFSET, connectRing.area);
        }
    }
    ...
    private void drawConnectRingStar(Canvas canvas) {
        ...
        for (int i = 0; i < mConnectStarNum; i++) {
            ...
            x = (float) (centerX + (radius + offsetRadius) * Math.cos((angles + mConnectStarDegreesPreOffset - offsetDegrees) / 180f * Math.PI));
            y = (float) (centerY + (radius + offsetRadius) * Math.sin((angles + mConnectStarDegreesPreOffset - offsetDegrees) / 180f * Math.PI));
            canvas.drawPoint(x, y, mColorPaint);
        }
    }

    private RectF offsetRectF(RectF rectF) {
        return new RectF();
    }
    ...
    private void startAnglesAnimator(int duration) {
        ...
        mAnglesAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
                angles = (float) animation.getAnimatedValue();
                invalidate();
            }
        });
